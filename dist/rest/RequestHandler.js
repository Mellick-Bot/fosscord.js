...
const { Events: { DEBUG, RATE_LIMIT, INVALID_REQUEST_WARNING }, browser, } = require('../util/Constants');
...
class RequestHandler {
    ...
    onRateLimit(request, limit, timeout, isGlobal) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const shouldThrow = typeof options.rejectOnRateLimit === 'function'
                ? yield options.rejectOnRateLimit(rateLimitData)
    ...
    execute(request) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (this.limited) {
                ...
                yield this.onRateLimit(request, limit, timeout, isGlobal); // eslint-disable-line no-await-in-loop
                // Wait for the timeout to expire in order to avoid an actual 429
                yield delayPromise; // eslint-disable-line no-await-in-loop
            }
            ...
            if (res.status === 401 || res.status === 403 || res.status === 429) {
                ...
                invalidCount++;
                const emitInvalid = this.manager.client.listenerCount(INVALID_REQUEST_WARNING) &&
    ...
    Sublimit: ${sublimitTimeout ? `${sublimitTimeout}ms` : 'None'}`);
                    yield this.onRateLimit(request, limit, timeout, isGlobal);
                    // If caused by a sublimit, wait it out here so other requests on the route can be handled
                    if (sublimitTimeout) {
                        yield Util.delayFor(sublimitTimeout);
                    }
                    return this.execute(request);
                }
